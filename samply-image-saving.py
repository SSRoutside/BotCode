...
with pyrs.Service() as a, pyrs.Device() as py_dev:
    # Start realsense (if available)
    py_dev.apply_ivcam_preset(0)
	...
	while True:
			...
			print('got here')
            		...
            		baseline_name = baseline_name + '_'+counter  #keeps count of the photo taken in this particular series
            		cv2.imwrite(baseline_name +'_'+ str(datetime.date.today())+'_'+ time.strftime("%H:%M:%S")+'_'+ '_depth.PNG', d_im_col);
            		cv2.imwrite(baseline_name +'_'+ str(datetime.date.today()) +'_'+ time.strftime("%H:%M:%S") +'_'+ '_color.PNG', rgb_im);
